...
**Status:** PC-0 through PC-4 are accepted. PC-4 final repair `b934f9d` was independently accepted by Codex on 2026-08-22 after focused verification (104/104 persistence locks; 33/33 revision-fence assertions). PC-5 — migration and installed smoke — is open. The accepted PC-4 commit is the rollback point for PC-5. Codex reviewed the PC-5 dry-run repair `c463fae` on 2026-08-22 and requested changes: the dry-run diagnosis is valid, but the cache workaround can certify an undurable or deleted backup as the pre-restore safety point. Installer/smoke remain blocked until backup creation has one durable owner and the failure/deletion cases are pinned.

PC-5's real-catalog dry run (all 8 steps of the coach's own protocol) is complete against an isolated, byte-verified copy of the real two-season catalog, never the live files: `tools/pc5-real-catalog-dry-run.mjs`, 36/36. The dry run found a genuine production defect — `SeasonStore.snapshot()`/`restoreBackup()` were completely non-functional on the real desktop app (see `GRIDIRON-IQ-PERSISTENCE-INVENTORY.md` §7f). Codex's independent review of the proposed cache repair at `c463fae` requested changes because it can report a deleted or not-durably-written backup as successful. **Repair that safety contract and re-review it before the remaining PC-5 steps (installer, installed smoke) proceed.**
