...
**Status:** **COMPLETE. PC-0 through PC-5 are accepted.** PC-4 final repair `b934f9d` was independently accepted by Codex on 2026-08-22 after focused verification (104/104 persistence locks; 33/33 revision-fence assertions). Codex accepted the structural PC-5 restore-point repair `166d28f`, then closed its final non-blocking rollback-capture edge across save/delete/backup with a focused **73/73** catalog-persistence proof. Coach smoke rejected **1.12.0-59** and **1.12.0-60** because neither close-hook implementation could close the installed desktop window. Replacement **1.12.0-61** flushes in JavaScript and then closes through a native Rust command. The coach confirmed an edit persisted across reopen and both title-bar and taskbar close paths work normally. Commit `ef6d1a1` / installer 1.12.0-61 is the accepted persistence rollback point.

That repair is closed structurally, with no cache anywhere: `CatalogPersistence.createBackup()` rolls back and refuses on a genuinely failed durable write (mirroring `deleteSeason()`'s own established rollback shape); `TauriBackend.createBackup()`'s cache is removed entirely; `TauriBackend.writeDisk()` reports its own internal backup result to its caller via an out-parameter, so `SeasonStore.snapshot()`/`saveNow()` never make a redundant second call for the same payload. See `GRIDIRON-IQ-PERSISTENCE-INVENTORY.md` §7g for full detail, mutation-verified evidence at every layer, and the re-run real-catalog dry run (`tools/pc5-real-catalog-dry-run.mjs`, 40/40, including a new Phase 5b that precisely isolates a real backup-specific `writeDb` failure against the real class stack).
