...
> branch remains `Codex/football-film-analyzer-GRiCW` (renaming those would
...
**Branch**: `Codex/football-film-analyzer-GRiCW`
...
├── vision-analyzer.js        # Codex Vision API integration
...
## Codex Vision API Integration

**File**: `js/vision-analyzer.js`

Sends video frames to Codex's vision API for AI-assisted tagging. Currently a **suggestion tool** — accuracy is limited by what a general vision model can determine from static frames. Manual tagging is the primary workflow.

**How it works**:
1. Extracts 8 JPEG frames at key moments (pre-snap through result)
2. Sends frames with a detailed football analysis prompt to Codex
...
- Default: `Codex-opus-4-6` with `anthropic-version: 2025-04-15`
...
1. **Video-native models** (Gemini, Codex with video, fine-tuned sport models)
...
3. **Commit + push** the bump to the feature branch
   (`Codex/football-film-analyzer-GRiCW`).
...
> origin/Codex/football-film-analyzer-GRiCW` (verify `git status` is clean
...
- **Codex Vision auto-tagging** → `api.anthropic.com` (needs API key; by design).
