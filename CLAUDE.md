...
```javascript
{
  ...
  tags: {
    ...
    yardLine: '',       // 1-50
    formation: '',      // MULTI-SELECT (offense). One or more of 'Shotgun' | 'Under Center' | 'Pistol' | 'I-Form' | 'Singleback' | 'Trips' | 'Spread' | 'Split Back' | 'Single Wing' | 'Empty' | 'Wildcat' | 'Goal Line' | 'Wing-T' | 'Flexbone' | 'Wishbone' | 'Double Wing' | 'Power-I' | 'Bunch' | 'Unbalanced', stored as a " + "-joined string (e.g. 'Pistol + Spread', 'Wing-T + Unbalanced'). Chips are grouped/ordered by QB alignment (Under Center · Pistol · Shotgun) → named/system formations → backfield & receiver sets (display order only; the field is order-independent). Analytics split on " + " and attribute the play to each component formation. ChipField({multi:true}); StatsEngine.splitFormations() is the canonical splitter.
