...
const initial = await page.evaluate(() => ({
  ...
  add: !!document.getElementById('btnAddAngle'),
  hidden: !document.getElementById('videoContainer')?.classList.contains('has-angle2'),
}));
ok(initial.mounted && initial.add && initial.hidden,
...
const loaded = await page.evaluate(() => {
  ...
  return {
    ...
    name: document.getElementById('angle2Name')?.textContent,
    ...
    controlsOpen: document.getElementById('angleControls')?.classList.contains('ang-open'),
...
ok(loaded.enabled && loaded.name === 'endzone.mp4' && /blob:test\/endzone\.mp4/.test(loaded.src || '')
  && loaded.rate === 1.5 && loaded.synced === 12 && loaded.sideBySide && loaded.controlsOpen
...
const sync = await page.evaluate(() => {
  ...
  const offset = document.getElementById('angleSyncOffset');
  offset.value = '1.5';
...
const views = await page.evaluate(() => {
  const multi = window.app.multiAngle;
  const container = document.getElementById('videoContainer');
  const select = document.getElementById('angleViewMode');
  select.value = 'pip';
...
ok(views.pip && views.clickSwap === 2 && views.keySwap === 1 && views.activeClass
  ...
  'PiP click and V-key swap the active camera through the production controls', JSON.stringify(views));

const removed = await page.evaluate(() => {
  const multi = window.app.multiAngle;
  document.getElementById('btnRemoveAngle').click();
  const container = document.getElementById('videoContainer');
  return {
    enabled: multi.enabled,
...
ok(!removed.enabled && removed.active === 1 && removed.name === null && removed.src === null
  && !removed.hasAngle && !removed.controlsOpen
