...
r = await page.evaluate(() => ({ kpis: document.querySelectorAll('.ws-study-kpis>div').length, bars: document.querySelectorAll('.ws-study-bar-row').length, linked: !!document.querySelector('.ws-study-bar-row[data-study-row]') }));
ok(r.kpis === 3 && r.bars > 0 && r.linked, 'Study renders KPI and film-linked effectiveness visuals', JSON.stringify(r));
...
r = await page.evaluate(() => ({ deltas: document.querySelectorAll('.ws-study-delta-row').length, linked: !!document.querySelector('.ws-study-delta-row[data-study-row]') }));
ok(r.deltas > 0 && r.linked, 'Comparison renders film-linked positive/negative delta visuals', JSON.stringify(r));
