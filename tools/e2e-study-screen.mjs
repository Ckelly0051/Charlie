...
r = await page.evaluate(() => ({ open:document.querySelector('.ws-plan-picker')?.open, plans:window.app.storage.seasonStore.plans().length, target:document.querySelector('#wsStudyPlanTarget')?.value, nameVisible:!document.querySelector('.ws-plan-picker-name')?.hidden }));
ok(r.open && r.plans === 0 && r.target === '__new__' && r.nameVisible, 'Save to Plan asks for an intentional destination before mutating season data', JSON.stringify(r));
...
r = await page.evaluate(() => { const p=window.app.storage.seasonStore.plans()[0]; return { plans:window.app.storage.seasonStore.plans().length, name:p?.name, items:p?.items.length, refs:p?.items[0]?.refs.length, kind:p?.items[0]?.kind, active:window.app.planScreen.activeId }; });
ok(r.plans === 1 && r.name === 'Rival Week' && r.items === 1 && r.refs > 0 && r.kind === 'finding' && r.active, 'Study creates the named plan and preserves its exact linked finding', JSON.stringify(r));
await page.click('[data-study-action="save-plan"]');
r = await page.evaluate(() => ({ target:document.querySelector('#wsStudyPlanTarget')?.value, active:window.app.planScreen.activeId, nameHidden:document.querySelector('.ws-plan-picker-name')?.hidden }));
ok(r.target === r.active && r.nameHidden, 'The picker defaults to the active existing plan without hiding the new-plan option', JSON.stringify(r));
await page.click('[data-study-action="plan-picker-save"]');
r = await page.evaluate(async () => { const store=window.app.storage.seasonStore,p=store.plans()[0],removed=p.items[p.items.length-1]; const sameRefs=JSON.stringify(p.items[0].refs)===JSON.stringify(removed.refs); store.removePlanItem(p.id,removed.id); await store.persist(); return { items:p.items.length, sameRefs }; });
ok(r.items === 1 && r.sameRefs, 'Choosing an existing plan targets it exactly and keeps composite film refs unchanged', JSON.stringify(r));
...
const presentationWatch=await page.evaluate(()=>{const app=window.app;window.__oldPlanWatch=app.studyScreen._watch;window.__planPresentationCalls=[];app.studyScreen._watch=(refs,label)=>window.__planPresentationCalls.push({refs,label});document.querySelector('[data-plan-present-ref]').click();const result={calls:window.__planPresentationCalls,closed:!document.querySelector('.ws-plan-present')};app.studyScreen._watch=window.__oldPlanWatch;delete window.__oldPlanWatch;return result;});
