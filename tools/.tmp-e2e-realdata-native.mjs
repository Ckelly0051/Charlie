...
const VIEWS = [
  ['dashboard', async () => { await window.app.workspaceShell.show('reports'); const host=document.getElementById('wsReports'); const content=host?.querySelector('[data-native-report-content]'); if (host?.hidden || !content || !content.textContent.trim()) throw new Error('NATIVE REPORTS BLANK'); }],
