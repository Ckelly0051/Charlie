...
export class SqlCatalog {
  ...
  saveSeason(data) {
    ...
    this.db.run('BEGIN');
    try {
      // wipe this season's children (cascade handles games→plays/clips)
  ...
  deleteSeason(id) { this._run('DELETE FROM seasons WHERE id = ?', [id]); if (this.currentId === id) this.currentId = null; }
