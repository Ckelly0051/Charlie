...
const BUCKETS = [
  ...
  { id: 'twomin',    label: '2 Minute',        count: 6, filter: p => /2\s*min|two\s*min|hurry/i.test(p.tags.custom || '') },
  { id: 'fourmin',   label: '4 Minute',        count: 4, filter: p => /4\s*min|four\s*min|kill/i.test(p.tags.custom || '') },
];
...
export class CallSheetBuilder {
  ...
  _playLabel(p) {
    ...
    if (t.playName) parts.push(`"${t.playName}"`);
    if (t.runDirection) parts.push(t.runDirection);
    if (t.passConcept) parts.push(t.passConcept);
  ...
  _renderCallSheet(title, buckets, numFor, wristband) {
    ...
    const sections = buckets.map((b, bIdx) => {
      ...
      const rows = b.plays.map((p, pIdx) =>
        `<tr><td class="num">${numFor(bIdx, pIdx)}</td><td>${this._esc(this._playLabel(p))}</td></tr>`
      ).join('');
