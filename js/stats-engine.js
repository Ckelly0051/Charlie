...
export class StatsEngine {
  ...
  _frontCoverageCombos(plays) {
    ...
    plays.forEach(p => {
      ...
      combos[k].yards += yds;
      if (!this._isSuccessfulPlay(p)) combos[k].successes++;
      if (p.tags.result === 'Sack' || p.tags.result === 'Interception' ||
          p.tags.result === 'Fumble' || (yds < 0 && p.tags.result !== 'Sack'))
        combos[k].havoc++;
      if (StatsEngine.isRun(p)) combos[k].runs++; else combos[k].passes++;
    });
  ...
  _playActionStats(plays) {
    ...
    const paComps = paPlays.filter(p => p.tags.result === 'Gain' || p.tags.result === 'Touchdown' || p.tags.result === 'No Gain');
    const paAttempts = paPlays.filter(p => p.tags.result !== 'Sack').length;
    const paYards = paPlays.reduce((s, p) => {
      if (p.tags.result === 'Incomplete' || p.tags.result === 'Interception') return s;
      return s + (parseInt(p.tags.yardage) || 0);
    }, 0);
    const straightComps = straightDrops.filter(p => p.tags.result === 'Gain' || p.tags.result === 'Touchdown' || p.tags.result === 'No Gain');
    const straightAttempts = straightDrops.filter(p => p.tags.result !== 'Sack').length;
...
${covRows ? `<table><thead><tr><th>Coverage</th><th>#</th><th>Yds</th><th>Avg</th><th>Stop%</th></tr></thead><tbody>${covRows}</tbody></table>` : ''}`;
    ...
    if (ind.tacklers.length) {
      body += '<h3>Individual Tackles</h3><table><thead><tr><th>Player</th><th>Tkl</th><th>Solo</th><th>Ast</th><th>Sack</th><th>TFL</th></tr></thead><tbody>';
      ind.tacklers.forEach(tv => { body += `<tr><td>${this._playerLabel(tv.num)}</td><td>${tv.tackles}</td><td>${tv.solo}</td><td>${tv.assists}</td><td>${tv.sacks}</td><td>${tv.tfl}</td></tr>`; });
      body += '</tbody></table>';
    }
