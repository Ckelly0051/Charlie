 ...
 *
 * Cached by a stable identity built from the game's id plus its resolved
 * film mode/dir/first-file name — relinking or changing a game's film
 * changes that identity, so a stale thumbnail is never served for new film.
 * This is a session-scoped IN-MEMORY cache only (not persisted to disk);
 * a fresh app launch regenerates thumbnails once, which is a disclosed,
 * deliberate simplicity trade for this first pass.
 *
 * Every request carries the caller's own season token; a resolved thumbnail
 * for a season the coach has since left is silently dropped by the caller
 * (HomeScreen re-checks its own token before applying a result) rather than
 * painting a stale card.
 */
