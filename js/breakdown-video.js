...
export class BreakdownVideo {
  constructor(tagger) {
    ...
    this._mounted = false;
    // Mount generation. Every deferred callback (rAF, ResizeObserver) captures
    // this and bails if it changed, so work queued by one mount can never land
  ...
  mount() {
    ...
    this._gen++;
    this._mounted = true;   // set BEFORE _mount(): the guarded listeners it binds
                            // check this flag, and render() runs inside it.
    this._mount();
    return true;
  }
  ...
  _bindPlayer(handle) {
    ...
    try {
      ...
      // place() writes controls.style.top. restore() clears it — so an unguarded
      // frame queued here re-applies beta positioning to the CLASSIC controls
      // after teardown.
      const gen = this._gen;
      if (Number.isFinite(stored) && stored >= 0 && stored <= 1) {
        requestAnimationFrame(() => { if (gen === this._gen) place(stored); });
      }
    } catch {}
    if (typeof ResizeObserver !== 'undefined') {
      this.resizeObserver = new ResizeObserver(() => {
        // Guard the observer callback AND the frame it queues: disconnect()
        // stops new callbacks but cannot recall one already in flight.
        const gen = this._gen;
        if (!this._mounted || !Number.isFinite(this.controlRatio)) return;
        requestAnimationFrame(() => { if (gen === this._gen) place(this.controlRatio); });
      });
  ...
  render() {
    ...
    // Capture the track locally AND check the generation: the entry guard above
    // ran a frame ago, and restore() nulls this.track — re-reading it here threw
    // "Cannot read properties of null (reading 'querySelector')" on a fast
    // mount -> render -> restore.
    const gen = this._gen, track = this.track;
    requestAnimationFrame(() => {
      if (gen !== this._gen) return;
      track.querySelector('.active')?.scrollIntoView({ block: 'nearest', inline: 'center' });
    });
  }
