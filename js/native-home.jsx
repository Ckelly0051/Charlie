...
function Thumbnail({ screen, game, detail = false }) {
  ...
  useEffect(() => { screen.requestThumbnail(game); }, [game.id, game.filmMode, game.filmDir]);
...
function NoSeasonPanel({ screen, c }) {
  return <div class="ws-empty-panel">
    <h3>{c.team ? 'Choose or create a season' : 'Set up your team first'}</h3>
    <p>{c.team ? 'Open an existing season or start a new one from the Season selector above.' : 'Team & Film Settings and the Season selector both get you there.'}</p>
...
function NativeHome({ screen }) {
  ...
  return <div class="ws-home-page" data-native-home>
    ...
    {state.status !== 'ready' ? null
      : !hasSeason ? <NoSeasonPanel screen={screen} c={c} />
        : !games.length ? <EmptySeasonPanel screen={screen} scout={scout} />
          : <GameWorkspace screen={screen} state={state} games={games} scout={scout} c={c} />}
  </div>;
}
