...
export class WorkspaceShell {
  ...
  async refreshHome() {
    ...
    this._text('wsGreeting',c.team?.name?`${c.team.name.toUpperCase()} HOME`:'TEAM HOME'); this._text('wsContinueTitle',c.game?.name||'No game open'); this._text('wsContinueMeta',c.game?`${c.game.playCount} plays · ${c.game.status.replace(/_/g,' ')}`:'Open a season to continue.');
    const resume=this.root.querySelector('#wsResume');
    if(c.capabilities.canBreakDown){resume.disabled=false;resume.textContent='Continue breakdown';resume.dataset.wsRoute='breakdown';delete resume.dataset.wsAction;}
    else{resume.disabled=false;resume.textContent=c.team?'Choose a season':'Set up team';delete resume.dataset.wsRoute;resume.dataset.wsAction='seasons';}
