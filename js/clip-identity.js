 ...
 *   4. `order` — only when NO stronger match exists and the same number of plays
 *                and clips remain, pair by sorted order. This is a wholesale-
 *                rename rescue, never a license to attach one unrelated leftover
 *                file after a partial exact match.
 *
...
export function planClipMatch(plays, clips) {
  ...
  if (!matches.length && remP.length && remP.length === remC.length) {
