...
function AnalysisSettings({ screen }) {
  const profile=screen.analysisProfile();const [key,setKey]=useState(profile.apiKey);const [model,setModel]=useState(profile.model);const [saved,setSaved]=useState(false);
  return <div data-settings-panel="analysis"><section class="gi-settings-section"><header><div><span class="gi-settings-kicker">OPTIONAL ANALYSIS</span><h3>Vision and local auto-detect</h3></div><span class="gi-settings-status">{profile.status}</span></header><div class="gi-settings-section-body"><p class="gi-settings-truth">Core charting and reports never require AI. These preferences only enhance optional frame analysis and play detection.</p><label class="gi-settings-field"><span>Claude API key</span><input type="password" value={key} onInput={e=>{setKey(e.currentTarget.value);setSaved(false);}} autocomplete="off" spellcheck="false" /></label><label class="gi-settings-field"><span>Vision model</span><select value={model} onChange={e=>{setModel(e.currentTarget.value);setSaved(false);}}><option value="claude-opus-4-6">Opus · most accurate</option><option value="claude-sonnet-4-6">Sonnet · faster</option></select></label><button type="button" class="gi-settings-primary" onClick={()=>{screen.saveAnalysis(key,model);setSaved(true);}}>Save analysis preferences</button>{saved&&<span class="gi-settings-saved" role="status">Analysis preferences saved</span>}</div></section></div>;
}
