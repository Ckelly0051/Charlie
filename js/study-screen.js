...
export class StudyScreen {
  ...
  _bind() {
    ...
    this.host.addEventListener('change', e => {
      if (e.target.id === 'wsStudySaved') { this._applyView(e.target.value); this._syncDeleteView(); }
        ...
        if (e.target.id === 'wsStudyScope') this._renderFilters();
        this.render();
      }
    });
  ...
  _playSets() {
    ...
    return { game: stamp(active), season: games.flatMap(stamp), prior: games.filter(game => String(game.id) !== activeId).flatMap(stamp), activeName: active ? this._gameName(active) : 'Current game' };
  }
  ...
  render() {
    ...
    state.compare ? this._renderCompare(result, state.measure) : this._renderQuery(result, state.scope, state.measure);
    this._renderWarnings(result.warnings || []);
  }

  _renderQuery(result, scope, measure) {
  ...
  _renderCompare(result, measure) {
    ...
    this._setWatchAll(aRefs, 'Watch current game');
  }
  ...
  _saveView() {
    ...
    const comparison = state.compare === 'prior' ? 'Game vs prior' : state.compare === 'season' ? 'Game vs season' : state.scope === 'game' ? 'Current game' : 'Season';
    ...
    const views = this._views();
    const id = `${state.dimension}|${state.scope}|${state.unit}|${state.measure}|${state.minSample}|${state.compare}|${JSON.stringify(state.filters)}`;
