...
export class PlayGrid {
  ...
  static PROMOTE_ON_COMMIT = { formation: 'qbAlignment', coverage: 'coverageFamily' };
  static OPTION_EXCLUDE = { formation: 'QB_ALIGNMENTS', coverage: 'COVERAGE_FAMILIES' };
  ...
  _options(col, current = []) {
    ...
    const exclude = PlayGrid.OPTION_EXCLUDE[col.key];
  ...
  _applyEdit(play, col, value) {
    if (col.key === 'notes') {
      ...
      const sibling = PlayGrid.PROMOTE_ON_COMMIT[col.key];
