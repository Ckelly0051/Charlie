...
function Spray({data}){const W=600,H=240,L=36,B=26,T=10,R=10,xOf=x=>L+x/100*(W-L-R),yOf=y=>T+(1-(y-data.yMin)/(data.yMax-data.yMin))*(H-T-B),zero=yOf(0);
  ...
  <line x1={L} y1={zero} x2={W-R} y2={zero} stroke="#4a5a7d" stroke-width="1.5" stroke-dasharray="4 3"/><text x={L-4} y={zero+3} fill="#8b949e" font-size="9" text-anchor="end">0</text><text x={L-4} y={yOf(data.yMax)+3} fill="#8b949e" font-size="9" text-anchor="end">{data.yMax}</text>
...
export function NativeOffenseVisualizations({plays}){const data=offenseVisualizationData(plays);if(!data||(!data.zones&&!data.spray&&!data.quarters))return null;return <Module title="Visualizations"><div class="viz-section">
  ...
  {data.quarters.map(row=><div class="viz-q-row" key={row.quarter}><span class="viz-q-name">{row.quarter}</span><div class="viz-q-bars"><div class="viz-q-bar run" style={{width:row.runPct+'%'}} title={`${row.runPct}% run`}/><div class="viz-q-bar pass" style={{width:(100-row.runPct)+'%'}} title={`${100-row.runPct}% pass`}/></div>
